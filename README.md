...
Fusion jobs may register a callback owner in `metadata.async_callback`:

```json
{
  "contract_version": "fusion-async-resume/v1",
  "enabled": true,
  "dify_conversation_id": "<sys.conversation_id>",
  "dify_user_id": "<sys.user_id>"
}
```

Relay never accepts a callback URL or arbitrary resume command from the job. The callback destination is fixed by Railway environment variables, and Relay deterministically derives the resume query from the Fusion stage:

- `fusion_corpus_ingest` -> `/fusion resume <job_id>` (result-only recovery; no file re-upload)
- `material_evidence_mapping` / `global_adjudication` -> `/fusion analyze`
- `scoped_decision` -> `/fusion decide <original decision_text>`
- Final Evidence Review / Synthesis / Quality / Repair stages -> `/fusion finalize`

On job terminal state, the worker moves `callback_status` to `pending`. A separate callback loop in the same worker claims callback deliveries with a lease, calls the same Dify Chatflow `/chat-messages` endpoint, and retries with bounded exponential backoff. Apply `sql/003_fusion_async_callbacks.sql` before deploying this version.

Railway shared variables required for automatic resume:

```text
DIFY_CALLBACK_ENABLED=true
DIFY_CALLBACK_BASE_URL=https://api.dify.ai/v1
DIFY_CALLBACK_API_KEY=<API key of this same Chatflow app>
```

The Dify DSL supplies the original `sys.conversation_id` and `sys.user_id`; the Relay API key and Dify app API key remain server-side secrets. Existing `relay_job_tool` 0.1.2 does not require a code change because it already forwards `request_json` opaquely.
